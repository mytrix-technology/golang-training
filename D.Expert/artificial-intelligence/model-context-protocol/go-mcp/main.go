...
func main() {
	...
	err := server.RegisterTool("hello", "say hello Yudhis", func(argument *MyFuncArgument) (*mcp_golang.ToolResponse, error) {
		return mcp_golang.NewToolResponse(mcp_golang.NewTextContent(fmt.Sprintf("Hello %Server!", argument.Submitter))), nil
	})
	if err != nil {
		panic(err)
	}
